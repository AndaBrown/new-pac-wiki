...
**为保证每位用户都能够分配到一定的带宽资源，以下账号进行会进行单线程限速处理。为预防SSR账号不会因为使用人数猛增而导致账号没法使用，每个SSR账号会设置同时在线最大设备数。如果自己搭建服务器就没有这个限制。**

**2017年10月18日：增加一个SSR账号。前三个SSR账号单线程限速80kb/s，最大同时在线设备数150台。最后一个账号单线程限速40kb/s，最大同时在线设备数300台。SSR协议为必填内容才能正常使用以下账号，即请使用SSR客户端。**

![](https://i.imgur.com/44QwiXT.png)
