...
将上面两行内容写入到新建文本文件中，注意：-k="密码"和-up="服务器地址:8100" 是服务端的配置信息，-l=":8100" 指的是客户端监听本地端口 8100，默认就行。第二行代码的作用是，当你某个地方/代码配置错误，CMD批处理不会运行后直接关闭，免得看不到报错信息。然后重命名新建的文本文件为：run.bat，并运行BAT脚本（或者以管理员身份运行），提示内容大概如下：
