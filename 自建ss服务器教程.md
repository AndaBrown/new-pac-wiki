...
购买vps服务器时，服务器地址优先选择：美国硅谷（Silicon Valley）、美国达拉斯(Dallas)、日本东京（Tokyo）、美国洛杉矶（Los Angeles）、新加坡（Singapore）等。**选择CentOS 6.X64位的系统（推荐。系统版本不要选的太高，不然装不上！）**。完成购买后，找到系统的密码记下来，部署服务器时需要用到。如图：
